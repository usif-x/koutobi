    ...
    <div v-else-if="book" class="container mx-auto z-90 py-12">
      ...
      <div class="bg-white rounded-2xl shadow-md overflow-hidden">
        <div class="flex flex-col lg:flex-row">
          ...
          <div class="w-full lg:w-3/5 xl:w-2/3 p-6 z-20">
            <!-- Book info -->
            <div class="mb-8">
              ...
              <div class="border-t border-b border-gray-100 py-6 mb-6">
                <div class="flex flex-col sm:flex-row items-center gap-4">
                  ...
                  <div class="flex items-center border border-gray-300 rounded-lg overflow-hidden">
                    ...
                    />
                    <button 
                      @click="quantity < book.stockCount ? quantity++ : null"
                      class="w-10 h-10 flex items-center justify-center text-gray-600 hover:bg-gray-100 transition-colors"
...
const { errorToast, successToast } = useAlerts(); // Added successToast import
...
const addToCart = async () => {
  ...
  try {
    ...
    if (error.response?.status === 401) {
      errorToast('يرجى تسجيل الدخول أولاً');
      router.push('/auth/login?redirect=' + encodeURIComponent(route.fullPath));
    } else if (error.response?.data == 'Maximum quantity of 10 per product exceeded') {
      errorToast('لا يمكن إضافة أكثر من 10 قطع من نفس المنتج');
    } else {
